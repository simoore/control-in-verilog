...
class NonlinearFunction(object):
    ...
    def __init__(self, func, iw, if_, name='nonlinear_function'):
        """
        Parameters
        ----------
        func : function: ndarray -> ndarray
            The function to be implemented in a lookup table in verilog. This
            parameter is function pointer. Its only input is an ndarray and it 
            must return an ndarray of the same length.
        iw : int
            The input word length. This value along with the fraction word 
            length parameter sets the input domain of the function.
        if_ : int
